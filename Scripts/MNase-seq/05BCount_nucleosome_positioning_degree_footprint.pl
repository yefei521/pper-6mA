...
use strict;
my (@bed,@sam,$sequence,$chr,$mid,$mid_bed,$n,$degree,%sam,%bed,$name,$name1,$bed_name,$name2,$location1,$location2,$keys);
open (IN1, "<", "$ARGV[0]")
		or die "Can't open IN1 : $!";
open (IN2, "<", "$ARGV[1]")
		or die "Can't open IN2 : $!";
open (OUT1, ">", "$ARGV[0]_degree_footprint");
{
		@bed= <IN1>;
		close IN1;
		@sam= <IN2>;
...
print OUT1"chrosome name\tNucleosome mid\tfragment number in ±75 footprint\n";

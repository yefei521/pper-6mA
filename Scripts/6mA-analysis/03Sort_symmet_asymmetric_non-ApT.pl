...
use strict;
my (@Result,$sequence,$line,$location,$ori,$asymmetric,$else_number,$n,@AT,$symmetric_number,$asymmetric_number,$name,$scafford,$start,$begin,%hash);
open (IN1, "<", "$ARGV[0]")
		or die "Can't open IN1 : $!";
open (OUT1, ">", "$ARGV[0]_non-ApT");

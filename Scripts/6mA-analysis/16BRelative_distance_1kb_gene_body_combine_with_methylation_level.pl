#!/usr/bin/perl -w
#This scripts used for analysis the 6mA in the genome,compare with genes.(Density, combine with methylation level)
#Usage: perl perl.pl mRNA_well_modeled_Jie_1kb_up.gff3 m6A.gff
use strict;
...
open (OUT1, ">", "$ARGV[1]_genebody_bin_distribution");
{
		@GFF= <IN1>;
		close IN1;
		@m6A= <IN2>;
	...
	if($sequence=~/(tig.+?)\t.+?m6A\t(\d+?)\t.+?frac=(.+?);/){
		...
		foreach $anotation(@GFF){
			...
			if ($location>=$five && $location <= $three){
				...
				$bin_number{$number}=$bin_number{$number}+$level;}
				elsif($ori eq "-") {

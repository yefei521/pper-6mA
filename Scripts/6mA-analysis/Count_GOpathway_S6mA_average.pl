...
foreach(<IN1>){ my@temp=split; $m6A{"S"}{$temp[3]}=$temp[7];     }
foreach(<IN3>){ my@temp=split; $m6A{"AS"}{$temp[3]}=$temp[7];    }
...
open(OUT1,">$dir/veg-cyst_GOpathway-total-gene_S6mA_amount.txt");
foreach(keys%py){
